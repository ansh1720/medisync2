...
function RiskAssessment() {
  ...
  useEffect(() => {
    if (location.state) {
      const { prefilledSymptom, diseaseQuery, searchQuery } = location.state;
      
      if (prefilledSymptom) {
        // Find matching symptom in SYMPTOMS array
        const matchingSymptom = SYMPTOMS.find(symptom => 
          symptom.label.toLowerCase().includes(prefilledSymptom.toLowerCase()) ||
          symptom.id.toLowerCase().includes(prefilledSymptom.toLowerCase())
        );
        
        if (matchingSymptom) {
          setFormData(prev => ({
            ...prev,
            symptoms: [matchingSymptom.id]
          }));
          setSearchHighlight(matchingSymptom.id);
          toast.success(`Pre-selected "${matchingSymptom.label}" from your search`);
        }
      }
      
      if (diseaseQuery) {
        setSearchHighlight(diseaseQuery);
        toast.info(`Showing risk assessment for "${diseaseQuery}"`);
      }
    }
  }, [location.state]);
          ...
          {location.state?.searchQuery && (
            <div className="mt-4 inline-flex items-center px-4 py-2 bg-blue-50 border border-blue-200 rounded-full">
              ...
              <span className="text-sm text-blue-700">
                Assessment for: <strong>"{location.state.searchQuery}"</strong>
              </span>
            </div>
          )}
        </div>
                      ...
                      {symptoms.map((symptom) => (
                        <label
                          ...
                          className={`
                            ...
                            ${searchHighlight === symptom.id ? 'ring-2 ring-yellow-400 ring-opacity-75' : ''}
                          `}
                        >
                          <input
                            type="checkbox"
